...
class BridgeCliTests(unittest.TestCase):
    ...
    def test_harness_install_skill_writes_skill_and_local_links(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("## New Project Location", skill_text)
            self.assertIn("Create, clone, scaffold, or initialize every new project", skill_text)
            self.assertIn("`~/Code/<project-name>`", skill_text)
            self.assertIn("`%USERPROFILE%\\Code\\<project-name>`", skill_text)
            self.assertIn("Do not move or rename an existing", skill_text)
            self.assertIn("`~/.local/state/agent-bridge/managed-repos.json`", skill_text)
            self.assertIn("`%USERPROFILE%\\.local\\state\\agent-bridge\\managed-repos.json`", skill_text)
            self.assertNotIn(str(home), skill_text)
